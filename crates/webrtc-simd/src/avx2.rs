...
/// AVX2 elementwise max: out[i] = max(a[i], b[i])
///
/// # Safety
/// Caller must ensure AVX2 is available.
#[target_feature(enable = "avx2")]
pub(crate) unsafe fn elementwise_max(a: &[f32], b: &[f32], out: &mut [f32]) {
    unsafe {
        ...
        for i in 0..chunks {
            ...
            let va = _mm256_loadu_ps(a_ptr.add(offset));
            let vb = _mm256_loadu_ps(b_ptr.add(offset));
            let result = _mm256_max_ps(va, vb);
            _mm256_storeu_ps(out_ptr.add(offset), result);
        }
...
/// AVX2+FMA standard complex multiply-accumulate:
///   acc_re[i] += x_re[i]*h_re[i] - x_im[i]*h_im[i]
///   acc_im[i] += x_re[i]*h_im[i] + x_im[i]*h_re[i]
///
/// # Safety
...
pub(crate) unsafe fn complex_multiply_accumulate_standard(
    ...
    unsafe {
        ...
        for i in 0..chunks {
            ...
            // acc_re += x_re*h_re - x_im*h_im (FMA + FNMA)
            let mut var = _mm256_loadu_ps(ar_ptr.add(offset));
            var = _mm256_fmadd_ps(vxr, vhr, var);
            var = _mm256_fnmadd_ps(vxi, vhi, var);
            _mm256_storeu_ps(ar_ptr.add(offset), var);

            // acc_im += x_re*h_im + x_im*h_re (two FMAs)
            let mut vai = _mm256_loadu_ps(ai_ptr.add(offset));
            vai = _mm256_fmadd_ps(vxr, vhi, vai);
            vai = _mm256_fmadd_ps(vxi, vhr, vai);
            _mm256_storeu_ps(ai_ptr.add(offset), vai);
        }
