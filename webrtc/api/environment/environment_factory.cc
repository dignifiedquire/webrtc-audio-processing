...
// Null implementations for components not needed in webrtc-audio-processing
class NullFieldTrials : public FieldTrialsView {
 public:
  std::string Lookup(absl::string_view key) const override { return ""; }
};

class NullTaskQueueFactory : public TaskQueueFactory {
 public:
...
Environment EnvironmentFactory::CreateWithDefaults() && {
  if (field_trials_ == nullptr) {
    Set(std::make_unique<NullFieldTrials>());
  }
  if (clock_ == nullptr) {
    Set(Clock::GetRealTimeClock());
  }
  if (task_queue_factory_ == nullptr) {
    Set(std::make_unique<NullTaskQueueFactory>());
  }
